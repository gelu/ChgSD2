...
struct MANGOS_DLL_DECL boss_forgemaster_garfrostAI : public ScriptedAI
{
    ...
    void Reset()
    {
        if(pInstance) pInstance->SetData(TYPE_GARFROST, NOT_STARTED);
    }

    void Aggro(Unit *who) 
    {
        if(pInstance) pInstance->SetData(TYPE_GARFROST, IN_PROGRESS);
    }

    void JustDied(Unit *killer)
    {
        if(pInstance) pInstance->SetData(TYPE_GARFROST, DONE);
    }
