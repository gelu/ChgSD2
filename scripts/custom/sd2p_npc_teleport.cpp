...
namespace
{
    ...
    void ActionTeleport(Player * const pPlayer, Creature * const pCreature, const uint32 &DestId)
    {
		...
		if ( DestId == 200 )
		{
			if (pDest->m_Map != 1)
			{
				pCreature->MonsterSay("　超级炉石功能没有开启，你不能将家绑定在这里!　", LANG_UNIVERSAL);
				return;
			}
		    WorldLocation loc;
		    uint32 area_id;
			...
			if (pPlayer->GetMap()->IsDungeon() || pPlayer->GetMap()->IsBattleGroundOrArena() || pPlayer->GetMap()->IsRaid())
			{
				pCreature->MonsterSay("　炉石绑定失败，你不能在战场，竞技场或者副本中绑定炉石!　", LANG_UNIVERSAL);
				return;
			}
			pPlayer->SetHomebindToLocation(loc, area_id);
			pCreature->MonsterSay("　炉石已经绑定　!　", LANG_UNIVERSAL);
			return;
		}
		else if (DestId == 201)
		{
	...
	void ActionTeleport(Player * const pPlayer, Item * const pItem, const uint32 &DestId)
    {
		...
		if ( DestId == 200 )
		{
			if (pDest->m_Map != 1)
			{
				pPlayer->GetSession()->SendAreaTriggerMessage("　超级炉石功能没有开启，你不能将家绑定在这里　!　");
				return;
			}
		    WorldLocation loc;
		    uint32 area_id;
			...
			if (pPlayer->GetMap()->IsDungeon() || pPlayer->GetMap()->IsBattleGroundOrArena() || pPlayer->GetMap()->IsRaid())
			{
				pPlayer->GetSession()->SendAreaTriggerMessage("　炉石绑定失败，你不能在战场，竞技场或副本中绑定炉石　!　");		
				return;
			}
			pPlayer->SetHomebindToLocation(loc, area_id);
			pPlayer->GetSession()->SendAreaTriggerMessage("　炉石已经绑定　!　");
			return;
		}
		else if (DestId == 201)
		{
...
bool GossipHello_npc_teleport(Player * pPlayer, Creature * pCreature)
{
    ...
    if(pPlayer->isInCombat())
    {
        ...
        pCreature->MonsterSay(" 　你在战斗中,不能使用传送 !!", LANG_UNIVERSAL);
        return true;
    }
