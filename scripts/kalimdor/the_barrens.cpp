...
struct MANGOS_DLL_DECL npc_twiggy_flatheadAI : public ScriptedAI
{
    ...
    bool CanStartEvent(Player* pPlayer)
    {
        ...
        debug_log("½Å±¾¿â£º npc_twiggy_flathead event already in progress, need to wait.");
        return false;
    }
    ...
    void SetChallengers()
    {
        for(uint8 i = 0; i < 6; ++i)
        {
            ...
            if (!pCreature)
            {
                debug_log("½Å±¾¿â£º npc_twiggy_flathead event cannot summon challenger as expected.");
                continue;
            }
