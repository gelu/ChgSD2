...
bool GossipHello_npc_arthas(Player *player, Creature *mCreature)
{
      ...
      player->ADD_GOSSIP_ITEM(0, "I\'m ready start culling of stratholme", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);

            if(((npc_arthasAI*)mCreature->AI())->FinalFight == 1)
      player->ADD_GOSSIP_ITEM(0, "We are ready to Final Combat!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
...
bool Event2Com;
 ...
 void MoveInLineOfSight(Unit *who)
    {      
           if (Event2 == false && m_creature->IsWithinDistInMap(who, 20.0f))
              {
                Target = who;
                Event2Com = true;
              }
...
{
    npc_time_riftCSAI(Creature *c) : ScriptedAI(c) {Reset();
}
