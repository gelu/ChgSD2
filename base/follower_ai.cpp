        ...
        if (m_uiUpdateFollowTimer < uiDiff)
        {
            ...
            if (Player* pPlayer = GetLeaderForFollower())
            {
                if (HasFollowState(STATE_FOLLOW_RETURNING))
                {
                    ...
                    m_creature->GetMotionMaster()->MoveFollow(pPlayer, PET_FOLLOW_DIST, PET_DEFAULT_FOLLOW_ANGLE);
                    return;
                }
...
void FollowerAI::StartFollow(Player* pLeader, uint32 uiFactionForFollower, const Quest* pQuest)
{
    ...
    m_creature->GetMotionMaster()->MoveFollow(pLeader, PET_FOLLOW_DIST, PET_DEFAULT_FOLLOW_ANGLE);
